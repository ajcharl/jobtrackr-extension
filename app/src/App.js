...
import { api, getToken, getCurrentUser, logout as apiLogout, API_BASE } from "./api";
...
function AppInner(props) {
  // user is the logged-in user object, onLogout is the logout callback
  var user = props.user;
  ...
  useEffect(function () {
    async function loadGmailStatus() {
      var gmailStatus = await api.getGmailStatus();
      if (gmailStatus) {
  ...
  useEffect(function () {
    async function loadSuggestions() {
      if (gmailConnected) {
        var data = await api.getSuggestions("pending");
  ...
  useEffect(function () {
    ...
    if (params.get("gmail_connected") === "true") {
      ...
      setEmailSyncOpen(true);
      window.history.replaceState({}, "", window.location.pathname);
      api.getGmailStatus().then(function (s) { if (s) setGmailEmail(s.email || ""); });
    }
  ...
  useEffect(() => { if (currentView === "trash") loadTrash(); }, [currentView]); // eslint-disable-line
    ...
    <Box sx={{ display: "flex", minHeight: "100vh" }}>
          ...
          <Box sx={{ px: 1.5, py: 1, borderTop: "1px solid", borderColor: "divider" }}>
            <Stack direction="row" alignItems="center" spacing={1}>
              ...
              <Tooltip title="Sign out" placement="right" arrow>
                <IconButton size="small" onClick={onLogout} sx={{ color: "text.secondary", width: 28, height: 28, "&:hover": { color: "error.main" } }}>
      ...
      <Box sx={{ flex: 1, ml: `${sidebarW}px`, transition: "margin 200ms ease" }}>
        <Box sx={{ maxWidth: 1200, mx: "auto", p: { xs: 2, sm: 3, md: 4 }, pt: { xs: 3, md: 4 } }}>
            ...
            <Box className="animate-in">
              ...
              <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 3 }}>
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                  <Tooltip title={gmailConnected ? `Gmail Sync (${suggestions.length} pending)` : "Connect Gmail"} arrow>
                    <IconButton
                      onClick={() => setEmailSyncOpen(true)}
                      sx={{
                        border: "1px solid", borderColor: "divider",
                        borderRadius: 2, width: 36, height: 36,
                        color: gmailConnected ? (isLight ? "#059669" : "#34d399") : "text.secondary",
          ...
          {/* ===== EMAIL SYNC DIALOG ===== */}
          <Dialog open={emailSyncOpen} onClose={() => setEmailSyncOpen(false)} maxWidth="sm" fullWidth
            PaperProps={{ sx: { borderRadius: 3, p: 1 } }}>
            <DialogTitle sx={{ fontSize: "1.125rem", fontWeight: 700, pb: 0.5 }}>
              Gmail Sync
            </DialogTitle>
            <DialogContent>
              <Typography sx={{ fontSize: "0.8125rem", color: "text.secondary", mb: 2.5 }}>
                Connect your Gmail to automatically detect application updates
              </Typography>
              ...
              <Paper elevation={0} sx={{
                ...
                <Stack direction="row" alignItems="center" spacing={1.5}>
                  <EmailIcon sx={{ fontSize: 20, color: gmailConnected ? (isLight ? "#059669" : "#34d399") : "text.secondary" }} />
                  <Box>
                    <Typography sx={{ fontSize: "0.875rem", fontWeight: 600, color: "text.primary" }}>Gmail</Typography>
                    <Typography sx={{ fontSize: "0.75rem", color: "text.secondary" }}>
                  ...
                  {gmailConnected ? (
                    <>
                      <Button size="small"
                        ...
                        sx={{ fontSize: "0.75rem", fontWeight: 600, bgcolor: isLight ? "rgba(37,99,235,0.06)" : "rgba(91,157,255,0.08)", color: isLight ? "#2563eb" : "#5b9dff" }}>
                    ...
                    <Button size="small" variant="contained" href={API_BASE + "/gmail/connect?token=" + encodeURIComponent(getToken() || "")}
                      sx={{ fontSize: "0.75rem", fontWeight: 600, background: isLight ? "linear-gradient(135deg, #111113, #2a2a2e)" : "linear-gradient(135deg, #e8e8ea, #d0d0d4)", color: isLight ? "#fff" : "#111113" }}>
                      Connect
                    </Button>
                  )}
                </Stack>
              ...
              {/* Sync result */}
              {syncResult && (
                <Alert severity="info" onClose={() => setSyncResult(null)} sx={{ mb: 2, borderRadius: 2, fontSize: "0.8125rem" }}>
              ...
              {suggestions.length > 0 && (
                <Box>
                  <Typography sx={{ fontSize: "0.875rem", fontWeight: 600, color: "text.primary", mb: 1.5 }}>
                          ...
                          <Stack direction="row" spacing={0.5} sx={{ mt: 1 }}>
                            {s.applicationId && (
                              <Button size="small" startIcon={<CheckIcon sx={{ fontSize: 14 }} />} onClick={() => handleApplySuggestion(s)}
                                sx={{ fontSize: "0.6875rem", fontWeight: 600, bgcolor: isLight ? "rgba(5,150,105,0.08)" : "rgba(52,211,153,0.1)", color: isLight ? "#059669" : "#34d399" }}>
                                Apply
                              </Button>
                            )}
                            <Button size="small" onClick={() => handleIgnoreSuggestion(s)} sx={{ fontSize: "0.6875rem", color: "text.secondary" }}>
                              Ignore
                            </Button>
              ...
              {gmailConnected && suggestions.length === 0 && !syncResult && (
                <Typography sx={{ fontSize: "0.8125rem", color: "text.secondary", textAlign: "center", py: 3 }}>
                  No pending suggestions. Click Sync to check for updates.
                </Typography>
              )}
            </DialogContent>
            <DialogActions sx={{ px: 3, pb: 2 }}>
              <Button onClick={() => setEmailSyncOpen(false)} sx={{ color: "text.secondary" }}>Close</Button>
            </DialogActions>
          </Dialog>
...
function App() {
  const [mode, setMode] = useState(function () { return localStorage.getItem("mui-mode") || "light"; });

  // user holds the logged-in user object, null means not authenticated
  var _userState = useState(null);
  var user = _userState[0];
  var setUser = _userState[1];

  // authLoading is true while we check if there's a valid token on startup
  var _authLoadingState = useState(true);
  var authLoading = _authLoadingState[0];
  var setAuthLoading = _authLoadingState[1];
  ...
  function handleLogin(userData) {
    setUser(userData);
    // Share the token with the Chrome extension if running inside one
    if (window.chrome && window.chrome.storage && window.chrome.storage.local) {
      window.chrome.storage.local.set({ jobtrackr_token: getToken() });
    }
  }
  ...
  function handleLogout() {
    apiLogout();
    setUser(null);
    // Clear the token from the Chrome extension storage too
    if (window.chrome && window.chrome.storage && window.chrome.storage.local) {
      window.chrome.storage.local.remove("jobtrackr_token");
    }
  }

  useEffect(function () { localStorage.setItem("mui-mode", mode); }, [mode]);
